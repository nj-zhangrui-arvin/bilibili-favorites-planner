...
```bash
node scripts/generate-review-page.mjs classification-review.jsonl review.html
```
...
```bash
node scripts/summarize-review.mjs reviewed-classification.jsonl review-summary.md
```
...
```bash
node scripts/build-task-package.mjs reviewed-classification.jsonl task-package.json
```
